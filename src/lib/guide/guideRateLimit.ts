// Server-side usage protection for the guide's one optional AI call.
//
...
//                     (CROSSHEARTPRAY_GUIDE_DAILY_ANONYMOUS_LIMIT, default 5).
//
...
const salt =
  process.env.CROSSHEARTPRAY_GUIDE_RATE_SALT || randomBytes(16).toString("hex");
...
export function checkRateLimit(key: string, now: number = Date.now()): RateDecision {
  ...
  if (bucket.dayCount >= getDailyAnonLimit()) {
