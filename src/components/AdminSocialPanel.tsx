...
// Client controls for one day's post package on /admin/social.
export default function AdminSocialPanel({
  adminKey,
  date,
  caption,
  imagePath,
  ...
  const imageSrc = imageVersion ? `${imagePath}?v=${imageVersion}` : imagePath;

  async function callPublishEndpoint(label: string, query: string) {
    setAction({ running: true, label, result: null });
    try {
      const response = await fetch(
        `/api/social/instagram/publish?date=${date}&${query}`,
  ...
  return (
    <div className="mt-4">
      {/* Regenerate re-renders the image from the SAME deterministic data —
          it can never change which verses were selected. */}
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-white/10">
        ...
        <img
          src={imageSrc}
          alt={`Daily Bible Bingo card for ${date}`}
